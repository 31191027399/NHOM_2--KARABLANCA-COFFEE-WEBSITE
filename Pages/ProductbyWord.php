                    ...
                    <div class="search-result">
                        <div class="grid_row">
                           ...
                           //Tách điều kiện tìm trang và điều kiện tìm kiếm sản phẩm trong ID
                            ...
                            if (isset($_GET['str'])) {
                                $mid = $_GET['str'];
                                if (!str_contains($mid, "page")) {
                                    $searchby = $mid;
                                    $trang = 1;
                                } else {
                                    $mid = str_replace("page", '#', $mid);
                                    $postition = strpos($mid, '#');
                                    $pagechar = strlen($mid) - $postition - 1;
                                    $searchby = substr($mid, 0, $postition);
                                    $trang = substr($mid, $postition + 1, $pagechar);
                                    settype($trang, "int");
                                }
                            ...
                            ?>
                            <?php
                            if (mysqli_num_rows($lenh_search) > 0) {
                                while ($row_search = mysqli_fetch_array($lenh_search)) { ?>
                                ...
                                <?php } ?>
                                <?php
                                $sql_trang = "SELECT sanpham.*,loaisp.tenloaisp FROM sanpham JOIN loaisp ON sanpham.maloaisp=loaisp.maloaisp
                                WHERE masp LIKE '%$searchby%' OR  tensp LIKE '%$searchby%' OR  
                                khoiluong LIKE '%$searchby%' OR gia LIKE '%$searchby%' OR tenloaisp LIKE '%$searchby%'";
                                ...
                                <ul class="pagination search-product__pagination">
                                    ...
                                    <?php for ($t = 1; $t <= $sotrang; $t++) { ?>
                                        <li class="pagination-item pagination-item--active">
                                            <a href="ProductbyWord.php?str=<?php echo "$searchby" ?>page<?php echo "$t" ?>" 
                                            class="pagination-item__link"><?php echo "$t" ?></a>
                                        <?php } ?>
                            ...
                            <?php } else {
                                echo "Không tìm thấy sản phẩm bạn cần";
                            } ?>
                        </div>
                    </div>
